...
def parse_arguments():
	...
	parser.add_argument('--show-coords', dest='show_coords_path', type=str, default='show-coords',
			help='Path to show-coords executable from MUMmer installation (default assumes already in PATH)')
	...
	if args['chrom_id'] is None: 
		args['chrom_id'] = get_default_chrom_id(args['genome_file'])
	return args
...
if __name__ == "__main__":
	...
	if args['correct_inv']:
		...
		os.system('{0} -r -l 15 -c 200 -g 20 -p {1}/tmp/nuc {2} {2}'.format(
			args['nucmer_path'], out_dir, tmp_genome_file))
		os.system('{0} -r -l -c {1}/tmp/nuc.delta > {1}/tmp/nuc.coords'.format(
			args['show_coords_path'], out_dir))
